...
import {
  IMAGE_TYPES, DOC_TYPES, CODE_EXTENSIONS, URL_PATTERN,
  blobToDataUrl, rawUpload, analyseImage,
...
export function useContentDrop({ addEntry }: UseContentDropOptions) {
  ...
  const processFile = useCallback(async (file: File): Promise<void> => {
    ...
    try {
      ...
      const attachment: FileAttachment = { name: file.name, mimeType: file.type, size: file.size, blobHash: hash };
      const notAborted = !abortRef.current.signal.aborted;

      if (IMAGE_TYPES.includes(file.type)) {
        const dataUrl = await blobToDataUrl(blob);
        addEntry({ type: 'image', dataUrl, alt: file.name });
        if (isGeminiAvailable() && notAborted) void analyseImage(dataUrl, file.type, addEntry);
        if (student && notebook && notAborted) void rawUpload(blob, file.name, student.id, notebook.id);
        return;
      }
      ...
      if (file.type.startsWith('text/') || file.type === 'application/json') {
        const text = await file.text();
        if (ext === 'md') addEntry({ type: 'prose', content: text });
        else addEntry({ type: 'code-cell', language: ext || 'text', source: text });
        if (student && notebook && notAborted) void rawUpload(blob, file.name, student.id, notebook.id);
        return;
      }
  ...
  const processPastedText = useCallback((text: string): boolean => {
    ...
    if (URL_PATTERN.test(trimmed)) { addEntry({ type: 'embed', url: trimmed }); return true; }
    return false;
  }, [addEntry]);
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (files.length > 0) { for (const file of files) void processFile(file); return; }
  ...
  const handlePaste = useCallback((e: React.ClipboardEvent): boolean => {
    ...
    if (imageItem) {
      ...
      if (file) { e.preventDefault(); void processFile(file); return true; }
    }
    const files = Array.from(e.clipboardData.files);
    if (files.length > 0) { e.preventDefault(); for (const file of files) void processFile(file); return true; }
    return false;
  }, [processFile]);
